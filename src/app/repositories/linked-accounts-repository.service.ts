...
import {
  LINKED_ACCOUNTS_SUBCOLLECTION_NAME,
  USER_COLLECTION_NAME,
} from '../constants/firestore/collection-names';
  ...
  implements ISubCollectionRepository<LinkedAccount>
{
  ...
  async get(
    ...
    const tmp = await super._get(
      LinkedAccountsRepositoryService.makeCollectionRef(userId),
      linkedAccountId
    );
  ...
  async getAll(): Promise<FirestoreDocumentQueryResult<LinkedAccount>> {
    ...
    if (tmp.docs) {
      tmp.docs = tmp.docs.map(
        (doc: LinkedAccount) =>
          this.convertTimestampToDate(doc) as LinkedAccount
      );
    }
    return tmp;
  }

  async getAllFromParent(
    userId: string
  ): Promise<FirestoreDocumentQueryResult<LinkedAccount>> {
    const tmp = await super._getAllFromParent(
      LinkedAccountsRepositoryService.makeCollectionRef(userId)
    );
    if (tmp.docs) {
      tmp.docs = tmp.docs.map(
  ...
  async delete(
    userId: string,
    linkedAccountId: string,
    shouldHardDelete: boolean = false
  ): Promise<boolean> {
  ...
  private convertTimestampToDate(
    ...
    const timestamp =
      linkedAccount.last_transaction_retrieval as unknown as Timestamp;
    if (timestamp)
      linkedAccount.last_transaction_retrieval = timestamp.toDate();
